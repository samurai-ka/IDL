...
set GEMDOS_TARGET_PATH=C:\temp\AtaDev\Hatari\gemdos-hd\IDL
set HATARI_BASE_PATH=C:\temp\AtaDev\Hatari\hatari-2.6.0_windows64
set VASM_BASE_PATH=C:\temp\AtaDev\vbcc

ECHO "Compiling %1.s%"
%VASM_BASE_PATH%\bin\vasmm68k_mot.exe -Ddebug=1 -no-opt -Ftos -noesc -o out\%1.tos %1.s

ECHO "Copying %1 to Gemdos drive..."
REM COPY /N /Y %USERPROFILE%\Sources\IDL\out\%1.tos %GEMDOS_TARGET_PATH%
COPY /N /Y out\%1.tos %GEMDOS_TARGET_PATH%
